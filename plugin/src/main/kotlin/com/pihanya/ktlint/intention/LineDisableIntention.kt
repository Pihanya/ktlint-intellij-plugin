...
class LineDisableIntention(private val error: LintError) : BaseIntentionAction() {
    ...
    override fun getText(): String {
        val errorRuleId = RuleId.fromString(error.ruleId)
        val ruleSetId = errorRuleId.ruleSetId

        val localeMessageId = when {
            ruleSetId.isExperimental() -> "ktlint.intention.disableExperimentalRule.line.text"
            else -> "ktlint.intention.disableRule.line.text"
        }
        val ruleIdRepresentation = when {
            ruleSetId.isStandard() || ruleSetId.isExperimental() -> errorRuleId.value
            else -> ruleSetId.asString()
        }
    ...
    override fun isAvailable(project: Project, editor: Editor?, file: PsiFile): Boolean {
        ...
        if (error.ruleId in unhandledErrors) return false
